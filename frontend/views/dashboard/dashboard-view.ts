...
import ChartSeries from '../../generated/org/vaadin/artur/offlineform/views/dashboard/ChartSeries';
...
export class DashboardView extends LitElement {
  ...
  firstUpdated(changedProperties: any) {
    ...
    this.statusColumn.renderer =
      // @ts-ignore
      (root, column, data) => (root.innerHTML = `<span theme="${data.item.theme}">${data.item.status}</span>`);

    // Retrieve data from the server-side endpoint.
    viewEndpoint.healthGridItems().then((items) => (this.grid.items = items));
